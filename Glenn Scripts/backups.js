...
function printOutSuggestions(ilNumbers) {
 Session.Output("--------------------------------------");
 Session.Output("The next available IL Number would be: IL." + (ilNumbers[ilNumbers.length - 1] + 1));
 Session.Output("--------------------------------------");
 Session.Output("Other numbers that are free for use by you:");
 
 for(var lastTriedNumber = 1; lastTriedNumber < ilNumbers[ilNumbers.length - 1]; lastTriedNumber++) {
  if(ilNumbers.indexOf(lastTriedNumber) == -1) {
   Session.Output("IL." + lastTriedNumber);
  }
 }
 
 Session.Output("--------------------------------------");
 Session.Output("Again, next available IL Number would be: IL." + (ilNumbers[ilNumbers.length - 1] + 1));
 Session.Output("--------------------------------------");
}
 
function findNumbersInUse() {
 var ils = Repository.GetPackageByGuid("{999E603A-E75B-4f07-A013-87D44B5BBC41}").Elements;
 var ilNumbersInUse = [];
 for(var i = 0; i < ils.Count; i++) {
  ...
  if("AA::Integratielijn" !== il.FQStereotype)
   continue;
  
  if(!il.Name)
   continue;
  
  if(il.Name.indexOf("IL.") === -1)
   continue;
  
  if(il.Name.length < 6)
   continue;
  
  var ilNumber = il.Name.substring(3,6);
  
  if(isNaN(ilNumber))
   continue;  
  
  ilNumbersInUse.push(ilNumber); 
 }
 
 return new Uint16Array(ilNumbersInUse).sort();
}
 
//Session.Output("Let's find you some IL Numbers for use..");
//Session.Output("Grab a coffee, give me a bit..");
//printOutSuggestions(findNumbersInUse());
//Session.Output("Voila, sie!");
