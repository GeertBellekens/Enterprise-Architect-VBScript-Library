...
Class MessageNode 
	...
	public property Get IsRedefined
		IsRedefined = false 'default value
		'check if there is a "redefine" stereotype
		'debug
'		if me.Name = "ClaimsIdentifier" then
'			Session.Output "found"
'		end if
		if not me.ElementOwner is nothing then
			if instr(lcase(me.ElementOwner.StereotypeEx), "redefine") > 0 then
				IsRedefined = true
				exit property
			end if
		end if
		if not me.sourceElement is nothing then
			if instr(lcase(me.sourceElement.StereotypeEx), "redefine") > 0 then
				IsRedefined = true
				exit property
			end if
		end if
		if not me.sourceAssociation is nothing then
			if instr(lcase(me.sourceAssociation.StereotypeEx), "redefine") > 0 then
				IsRedefined = true
				exit property
			end if
		end if
		if not me.sourceAttribute is nothing then
			if instr(lcase(me.sourceAttribute.StereotypeEx), "redefine") > 0 then
				IsRedefined = true
				exit property
			end if
			'check the owner of the attribute
			dim attributeOwner as EA.Element
			set attributeOwner = Repository.GetElementByID(me.sourceAttribute.ParentID)
			if instr(lcase(attributeOwner.StereotypeEx), "redefine") > 0 then
				IsRedefined = true
				exit property
			end if
		end if
	...
	public property get isOneOfAttribute
		dim result
		result = false
		if me.SourceAttribute is nothing then
			exit property
		end if
		dim attributeOwner as EA.Element
		set attributeOwner = Repository.GetElementByID(me.sourceAttribute.ParentID)
		dim oneOfTagValue
		oneOfTagValue = getTaggedValueValue(attributeOwner, "compositionType")
		if oneOfTagValue  = "oneOf" then
			result = true
		end if
		'return
		isOneOfAttribute = result
	end property
...
'		end if
	...
	public function getStructureOutput(current_order,currentPath,messageDepth, exportType)
		...
		select case exportType
			...
			case mappingDocument
				'if this node is an attribute in a oneOf we need to skip it's own output and continue with the child nodes
	...
	function getThisNodeMappingOutput(current_order,currentPath, messageDepth)
		'don't return any output if this is a oneOf attribute
		if me.isOneOfAttribute then
			set getThisNodeMappingOutput = nothing
			exit function
		end if
		'get the list for this node
		dim currentNodeList
	...
	function getThisNodeFSOutput(current_order,currentPath, messageDepth)
		...
		if not me.ElementOwnerSourceType is nothing _
		  and not me.IsRedefined then
