...
##**CLASS MEMBERS**
**First things first**: The most frequently used Properties & Functions :

```vbs
Public Function Wrap(ByRef aTaggedValue) ''': EA.TaggedValue (the wrapped Tag);	Passing Tag's parent object as "aObj" causes a lookup of the desired TaggedValue (returns True if the TV exists). The TV's properties are available.
Public Function TryWrapByName(aName, ByRef aObj) ''': Boolean; 		Passing the Tag's parent object as "aObj" gives direct access to properties (assuming that the TV actually exists). 
...
Public Sub Update() ''': Void	''' All PropertyTypes reloaded from Repository. Total re-initialization
```
...
```vbs
	...
	Public Sub StatsStop() ''': Void		''' Keeps all count and time info until next StatsStart()
		...
		''' as to allow for "Type safe" traversing of EA models.
