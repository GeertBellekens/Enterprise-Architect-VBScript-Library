...
function addElementDetails(xmlClass, xmlDOM, xmlSchema, element, allNodes, processedNodes)
	...
	next
	'add subclasses for classes (not datatypes)
	if element.Type = "Class" then
		dim subClasses
		set subClasses = getSubClasses(element)
		dim subClass
		For each subClass in subClasses
			'add the node for the target
			createOrUpdateElementNode xmlDOM, xmlSchema, subClass, allNodes, processedNodes
		next
	end if
end function

function getSubClasses(element)
	
	dim sqlGetData
	sqlGetData = "select c.Start_Object_ID as Object_ID from t_connector c " & vbNewLine & _
				" where c.Connector_Type = 'Generalization'                " & vbNewLine & _
				" and c.End_Object_ID = " & element.ElementID
	dim result
	set result = getElementsFromQuery(sqlGetData)
	'return
	set getSubClasses = result
end function
