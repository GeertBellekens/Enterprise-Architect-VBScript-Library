...
function createElementNode(xmlDOM, xmlSchema, element, allNodes)
	...
	for each relation in element.Connectors
		...
		or relation.Type = "Aggregation" ) then
			...
			'get the target element according to the direction of the association
			if relation.ClientID = element.ElementID _
			and relation.Direction <> "Destination -> Source" then
				set targetElement = Repository.GetElementByID(relation.SupplierID)
			elseif relation.SupplierID = element.ElementID _
			and relation.Direction = "Destination -> Source" then
				set targetElement = Repository.GetElementByID(relation.ClientID)
			end if
