...
const PK_Template_RisksAndRequirements = "PK_RisksAndRequirements"
const PK_Template_Issues = "PK_Issues"

'*** update GUID to match the GUID of the 'Documents' package of this model ***
const DocumentsPackageGUID = "{41D64E1D-4473-44b9-9220-B2E1EFD30ABF}"
'***************************************************************************************************
...
function createBPDocument(element)
 ...
 if element.Stereotype = "BusinessProcess" Then
  'remember all processes and subProcesses
  dim processes
  set processes = CreateObject("System.Collections.ArrayList")
  'add the process
 ...
 '  end if 
   ...
   for each subProcess in subProcesses
    ...
    if subProcess.Stereotype = "Activity" _
     ...
     if not subdiagram is nothing Then  
      'get the "real" subprocess if needed
      if subdiagram.ParentID > 0 _
        and subdiagram.ParentID <> subProcess.ElementID then
       set subProcess = Repository.GetElementByID(subdiagram.ParentID)
   ...
   '********************************************************* OPEN ISSUES **************************************************************
   'add model document with risks and requirements
   Session.output "Adding Risks and Requirements"
   addModelDocumentForPackage masterDocument,bpPackage, "Risks and requirements", i, PK_Template_RisksAndRequirements
   i = i + 1
   
   'add open issues per process/subProcess
   Session.output "Adding Open Issues"
   dim processElement as EA.Element
   for each processElement in processes
    if processHasIssues(processElement) then
     dim processPackage as EA.Package
     set processPackage = Repository.GetPackageByID(processElement.PackageID)
     'check if there are any issues
     addModelDocumentForPackage masterDocument,processPackage, "Open Issues " & processElement.Name , i, PK_Template_Issues
     i = i + 1
    end if
   next
...
function processHasIssues(element)
 dim sqlGetData
 sqlGetData = "select o.Object_ID                                                                                          " & vbNewLine & _
    ...
    " where o.Package_ID = (" & element.PackageID & ")                                                            " & vbNewLine & _
    " and o.Object_Type = 'Issue'                                                                                 " & vbNewLine & _
    " order by o.Name;                                                                                            "
 dim data
 set data = getArrayListFromQuery(sqlGetData)
 if data.Count > 0 then
  processHasIssues = true
 else
  processHasIssues = false
 end if
end function
...
function getConceptualBusinessProcesses(element)
 ...
 dim sql 
 'find related ArchiMate_BusinessProcesses or DeLijnProcess for BusinessProcess via Trace-relationship
 ...
 sql = "select o.Object_ID  "& _
   ...
   " where o.Stereotype in ('ArchiMate_BusinessProcess','DeLijnProces')  "& _
...
function addConceptualBusinessProcesses(masterDocument,ArchiMateProcesses,bpPackage,element,i)
 ...
 if ArchiMateProcesses.Count > 0 then
  ...
  for each ArchiMateProcess in ArchiMateProcesses
   ...
   i = i + 1
   'get "parent" of archimate process
   dim parentProcess
   set parentProcess = getParentArchimateProcess (ArchiMateProcess)
   if not parentProcess is nothing then
    set compositeDiagram = parentProcess.CompositeDiagram
    if not compositeDiagram is nothing then
     'debug
     Session.Output "Found composite diagram for conceptual process " & ArchiMateProcess.Name
     ...
     if compositeDiagram.ParentID = 0 then
      ...
      dim compositeOwner as EA.Element
      set compositeOwner = Repository.GetElementByID(compositeDiagram.ParentID)
      addModelDocumentForElement masterDocument, compositeOwner, i, PK_Template_ConceptualProcessDiagramViaElement
     end if
     i = i + 1
    else
     'debug
     Session.Output "Did not find composite diagram for conceptual process " & ArchiMateProcess.Name
    end if
   end if
  next
 else
...
function getParentArchimateProcess (ArchiMateProcess)
 dim connector as EA.Connector
 dim parentProcess as EA.Element
 set parentProcess  = nothing 'initialize
 for each connector in ArchiMateProcess.Connectors
  if connector.Stereotype = "ArchiMate_Composition" then
   if connector.ClientEnd.Aggregation = 2 and connector.ClientID <> ArchiMateProcess.ElementID then
    set parentProcess = Repository.GetElementByID(connector.ClientID)
    exit for
   elseif connector.SupplierEnd.Aggregation = 2 and connector.SupplierID <> ArchiMateProcess.ElementID then
    set parentProcess = Repository.GetElementByID(connector.SupplierID)
    exit for
   end if
  end if
 next
 'return 
 set getParentArchimateProcess = parentProcess
end function
...
function getSubProcesses(parentpackage, diagram)
 ...
 for each diagramObject in sortedDiagramObjects
  ...
  elseif subProcess.Stereotype = "Activity" _
   ...
   Session.Output "Processing " & subProcess.Stereotype & " " & subProcess.Name & " has composite diagram: " & hasSubDiagram
