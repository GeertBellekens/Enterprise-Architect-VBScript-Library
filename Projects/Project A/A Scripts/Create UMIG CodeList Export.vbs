...
sub main
	...
	excelOutput.createTab "Code Lists", excelContents, true, "TableStyleMedium13"
	'save the excel file
	excelOutput.save
end sub
...
function getSQLGetCodeList()
	getSQLGetCodeList = "select distinct o.Name as Enumeration,'''' + a.Name as Value, isnull(acd.VALUE, inh.CodeName) as Description,        " & _
						"  ovr.Value as versionID,                																		      " & _
						" oca.Value +'_' + oid.Value + '_' + replace(ovr.Value, '.','p') + '.xsd' as FileName,                                " & _
						" inh.CodeListAgency +'_' + inh.UniqueID +'_' + replace(inh.VersionID, '.','p') + '.xsd' as InheritedFrom             " & _
						" from t_object o                                                                                                     " & _
						...
						" 			ogca.Value as CodeListAgency, ogid.Value as UniqueID, ogvr.Value as VersionID,agcd.Value as CodeName      " & _
						...
						" 			inner join t_objectproperties ogid on ogid.Object_ID = og.Object_ID                                       " & _
						" 												and ogid.Property = 'uniqueID'                                        " & _
						" 			inner join t_objectproperties ogvr on ogvr.Object_ID = og.Object_ID                                       " & _
						" 												and ogvr.Property = 'versionID'                                       " & _
