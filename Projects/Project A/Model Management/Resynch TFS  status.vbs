...
'
' Script Name: Resync TFS status
' Author: Geert Bellekens
' Purpose: Does Resynch Status on all version controlled packages in the tree.
' Date: 2019-01-25
'

const outPutName = "Resync TFS status"
...
sub main
	...
	Repository.WriteOutput outPutName, now() & " Starting Resync TFS status for package '"& package.Name &"'", 0
	'ask the user if he is sure
	dim userIsSure
	userIsSure = Msgbox("Do you really want resync Status and XMI Reconcile with TFS '" &package.Name & "' ?", vbYesNo+vbQuestion, "Get latest from TFS?")
	if userIsSure = vbYes then
		'actually do the getlatest
		resynchFromTFS package
	...
	Repository.WriteOutput outPutName, now() & " Finished Resync TFS status for package '"& package.Name &"'", 0
end sub

function resynchFromTFS(package)
	'first process this package
	if package.IsVersionControlled then
		Repository.WriteOutput outPutName, now() & " Resync TFS status for package '"& package.Name &"'", 0
		dim isReadWrite
		...
		package.VersionControlResynchPkgStatus true
	end if
